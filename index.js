...
*Layanan Lainya:*\n
- *10. STALK PROFIL TIKTOK*
- *11. CEK ID FREE FIRE ( LENGKAP! )*
- *12. Remini ( Editing Photos )*\n\n
> _100% GRATIS!!_
> _Bisa Digunakan Berulang Kali Untuk Layanan Medsos_\n
...
sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    if (userSelections[sender]?.serviceKey === 12) {
        if (imageMessage) {
            ...
            await remini(sock, msg); // Kirim gambar ke fungsi Remini
    ...
    if (text.toLowerCase() === "y") {
        ...
        await sock.sendMessage(sender, { 
            text: `*Mau order berapa? ( KETIK ANGKA SAJA )*\n\n*_( Max Order 50 )_*\n> *_contoh: order 10 Like × jumlah ordermu_*`
        });
        return;
...
if (userSelections[sender]?.step === "choose_service") {
    ...
    if (serviceKey === 10) {
        ...
        await sock.sendMessage(sender, { text: `*Mau order berapa? ( KETIK ANGKA SAJA )*\n\n*_( Max Order 50 )_*\n> *_contoh: order 10 Like × jumlah ordermu_*` });
    }
    return;
}
...
if (userSelections[sender]?.step === "choose_quantity") {
    ...
    if (msg.message?.imageMessage) {
        console.log("🖼 Gambar untuk Remini terdeteksi!");
        await remini(sock, msg); // Kirim gambar ke fungsi Remini
        delete userSelections[sender]; // Reset setelah diproses
        return;
    } else {
        await sock.sendMessage(sender, { text: "📷 Harap kirim gambar untuk diproses oleh Remini!" });
        return;
    }
    }
}
    ...
    if (userSelections[sender]?.step === "choose_quantity") {
        ...
        if (quantity > 50) {
            await sock.sendMessage(sender, { text: "*maksimal order cuma 50😒*" });
            return;
        }
